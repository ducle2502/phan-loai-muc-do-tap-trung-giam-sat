...
def main():
 ...
 if any(x not in rows[0] for x in R):return 5
 issues=[]
 ...
 out={'status':'pass' if not issues else 'fail','rows_checked':len(rows),'error_count':len(issues),'issues':issues};text=json.dumps(out,ensure_ascii=False,indent=2);print(text)
